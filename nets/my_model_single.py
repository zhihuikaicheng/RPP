...
class MyResNet(BaseModel):
    def __init__(self, num_classes, sizes, scope, output_stride, spatial_squeeze, reuse, global_pool, is_training=True):
        ...
        with tf.variable_scope(scope):
            self.init_input()
            with slim.arg_scope(resnet_v2s.resnet_arg_scope()):
                self.init_network()
            self.init_loss()
...
class SubResNet(BaseModel):
    ...
    def init_network(self):
        ...
        net, end_points = resnet_v2s.resnet_v2s_50(
            x,
        ...
        # self.logits = end_points['resnet_v2s_50/branch_0/resnet_v2s_50/spatial_squeeze']
    ...
    def load_pretrain_model(self, sess, path, father_scope):
        '''
        in pretrain, name like:InceptionV3/Mixed_7b/Branch_2/Conv2sd_0d_3x1/weights
        in our model, name like:inception_v3/branch_0/InceptionV3/Mixed_7b/Branch_2/Conv2sd_0d_3x1/weights:0
        ...
        in pretrain, name of these vars like:InceptionV3/AuxLogits/Conv2sd_2b_1x1/biases
                                             InceptionV3/Logits/Conv2sd_2b_1x1/biases
        ...
        for var in variables:
            ...
            if name.startswith('resnet_v2s_50/logits') or name.startswith('pcb'):
                continue
            d[name] = var
